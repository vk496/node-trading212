...
class Trading212 extends EventEmitter {
    constructor(ENV, LOGIN_TOKEN, dUUID) {
        super();
        ...
        this.transport
            .get(baseUrl + "/rest/v1/webclient/authenticate").then(res => {

                this.headers["Cookie"] = res.headers["set-cookie"].map(f => { return f.match(/(.+?; )/)[0] }).join("")
                this.transport = axios.create({
                    withCredentials: true,
                    headers: this.headers
                });

                this.emit('refresh-token', res.data);
            })
            .catch(err => {
                throw new Error("Session cookie expired! : " + err)
            })

        var opts = { headers: this.headers };

        this.on("refresh-token", () => {
    ...
    getBucketId(bucketName, bucketID) {
        this.subscribeRoute('BUCKETS');
        this.on('buckets', (response) => {
